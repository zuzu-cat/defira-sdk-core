...
describe('CurrencyAmount', () => {
  ...
  describe('constructor', () => {
    it('works', () => {
      const token = new Token(1666600000, ADDRESS_ONE, 18)
  ...
  describe('#ONE', () => {
    it('produces ONE amount', () => {
      const amount = CurrencyAmount.fromRawAmount(ONE.onChain(1666600000), 100)
      expect(amount.quotient).toEqual(JSBI.BigInt(100))
      expect(amount.currency).toEqual(ONE.onChain(1666600000))
    })
  })

  it('token amount can be max uint256', () => {
    const amount = CurrencyAmount.fromRawAmount(new Token(1666600000, ADDRESS_ONE, 18), MaxUint256)
    expect(amount.quotient).toEqual(MaxUint256)
  })
  it('token amount cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromRawAmount(
        new Token(1666600000, ADDRESS_ONE, 18),
        JSBI.add(MaxUint256, JSBI.BigInt(1666600000))
      )
    ).toThrow('AMOUNT')
  })
  ...
  describe('#toFixed', () => {
    it('throws for decimals > currency.decimals', () => {
      const token = new Token(1666600000, ADDRESS_ONE, 0)
